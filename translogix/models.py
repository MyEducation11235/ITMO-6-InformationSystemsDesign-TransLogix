...
# Route status values:
#   pending              — создан оператором, ожидает водителя
#   in_progress          — водитель начал маршрут
#   completed            — все заказы выполнены успешно
#   requires_intervention — хотя бы один заказ не выполнен
...
class Warehouse(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            "lat": self.lat, "lon": self.lon, "contact": self.contact or "",
        }
...
class Route(db.Model):
    ...
    created_at     = db.Column(db.DateTime, default=datetime.utcnow)
    # pending / in_progress / completed / requires_intervention
    status         = db.Column(db.String(50), nullable=False, default="pending")

    stops = db.relationship(
        "RouteStop", backref="route", lazy=True,
        order_by="RouteStop.stop_order", cascade="all, delete-orphan"
    )
    ...
    def to_dict(self):
        return {
            ...
            "status_label":   ROUTE_STATUS_LABELS.get(self.status or "pending", self.status),
            "status_color":   ROUTE_STATUS_COLORS.get(self.status or "pending", "secondary"),
...
class RouteStop(db.Model):
    ...
    lon             = db.Column(db.Float, nullable=False)
    address         = db.Column(db.String(500), nullable=False)
    stop_type       = db.Column(db.String(50), nullable=False)  # 'warehouse' | 'delivery'
    # null = not yet marked; 'success' = доставлено; 'failure' = не доставлено
    delivery_result = db.Column(db.String(20), nullable=True)

    def to_dict(self):
        order_info = None
        if self.order_id and self.order:
            order_info = {
                "product_name": self.order.product.name if self.order.product else None,
                "product_unit": self.order.product.unit if self.order.product else None,
